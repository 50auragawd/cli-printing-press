...
func Recall(ctx context.Context, db *sql.DB, query string, opts Opts) (Result, error) {
	...
	// numeric-prefix or lowercase tokens because they don't match
	// its detection rules. Walk queryTokens — the tokens Normalize
	// classified as non-entity content — and promote any whose
	// lowercased form has a row in entity_lookups into
	// normalized.Entities. This lets the canonical resolution path
	// fire for aliases the literal extractor would miss.
	...
	if len(queryCanonicals) > 1 {
		...
		// resolves to multiple canonicals (e.g., a single alias
		// shared across multiple kinds). The agent can pass
		// --debug-mismatches to investigate.
	...
	for rows.Next() {
		...
		// lets a query alias and a stored alias match through their
		// shared canonical even when the literal strings differ.
		...
		//   2. Cross-alias fallback: canonicals overlap even when
		//      literal entities don't. Any canonical overlap counts;
		//      ambiguous aliases (one query token resolving to multiple
		//      canonicals) still pass since the Jaccard ratio would
		//      otherwise drop below threshold. The downstream
		//      validateResource still gates entity match against the
		//      stored resource, so ambiguous matches that hit the
		//      wrong canonical get caught at that layer.
		if score < jMin {
